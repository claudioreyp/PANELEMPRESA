...
export function BusinessDetailPage() {
  ...
  async function saveBusiness() {
    ...
    try {
      const updated = await api<Business>(`/admin/businesses/${businessId}`, { method: "PATCH", body: JSON.stringify({ name: businessForm.name, status: businessForm.status, plan: businessForm.plan, phone: businessForm.phone, modules: businessForm.modules }) });
  ...
  return <div className="page-stack detail-page">
    ...
    <section className="overview-strip"><article><Store /><span>Plan<strong>{businessForm.plan}</strong></span></article><article><GitBranch /><span>Sucursales<strong>{overview.branches}</strong></span></article><article><UserRoundCog /><span>Usuarios activos<strong>{overview.users}</strong></span></article><article><Building2 /><span>Pedidos históricos<strong>{overview.orders}</strong></span></article><article><ShieldCheck /><span>Ventas cerradas<strong><Money value={overview.sales} /></strong></span></article></section>
    <section className="detail-grid">
      <article className="panel config-card"><header><div><span className="eyebrow">Cuenta</span><h2>Plan y operación</h2></div><ShieldCheck /></header><div className="form-grid"><label>Nombre<input value={businessForm.name} onChange={(event) => setBusinessForm({ ...businessForm, name: event.target.value })} /></label><label>Teléfono<input value={businessForm.phone || ""} onChange={(event) => setBusinessForm({ ...businessForm, phone: event.target.value })} /></label><label>Plan<select value={businessForm.plan} onChange={(event) => setBusinessForm({ ...businessForm, plan: event.target.value })}><option value="basic">Básico</option><option value="pro">Pro</option><option value="superpro">SuperPRO</option></select></label><label>Estado<select value={businessForm.status} onChange={(event) => setBusinessForm({ ...businessForm, status: event.target.value as Business["status"] })}><option value="active">Activo</option><option value="suspended">Suspendido</option></select></label></div><div className="evidence-settings"><strong>Aprobación humana obligatoria</strong><small>El agente puede registrar una captura real y sus datos, pero solo un supervisor puede aprobar el pago y enviar el pedido a preparación.</small></div></article>
      <article className="panel module-card"><header><div><span className="eyebrow">Entitlements</span><h2>Módulos habilitados</h2></div><Check /></header><div className="module-toggle-list">{modules.map((module) => <label key={module}><span><strong>{module.toUpperCase()}</strong><small>{module === "whatsapp" ? "Preparado para la futura capa de IA" : "Visible para el equipo del restaurante"}</small></span><input type="checkbox" checked={Boolean(businessForm.modules[module])} onChange={(event) => setBusinessForm({ ...businessForm, modules: { ...businessForm.modules, [module]: event.target.checked } })} /></label>)}</div></article>
    </section>
    <section className="detail-grid">
    ...
    <IntegrationPanel branches={branches} onBranchUpdated={resource.refresh} />
    <section className="panel"><header><div><span className="eyebrow">Accesos</span><h2>Propietarios y empleados</h2></div><UserRoundCog /></header>{memberships.length ? <div className="responsive-table"><table><thead><tr><th>Usuario</th><th>Sucursal</th><th>Rol</th><th>Estado</th><th>Acción</th></tr></thead><tbody>{memberships.map((member) => <tr key={member.id}><td><strong>{member.full_name}</strong><small>{member.email}</small></td><td>{branches.find((branch) => branch.id === member.branch_id)?.name || "Todo el negocio"}</td><td><select value={member.role} onChange={(event) => void updateMembership(member, { role: event.target.value })}>{roles.map((role) => <option key={role}>{role}</option>)}</select></td><td><Status value={member.active ? "active" : "suspended"} /></td><td><button className={member.active ? "text-danger" : "text-success"} onClick={() => void updateMembership(member, { active: !member.active })}>{member.active ? "Suspender" : "Reactivar"}</button></td></tr>)}</tbody></table></div> : <Empty title="Sin usuarios activos" detail="Envía una invitación para incorporar al equipo." />}</section>
