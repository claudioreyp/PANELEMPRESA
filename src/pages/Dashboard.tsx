...
export function DashboardPage() {
  ...
  return <div className="page-stack">
    ...
    <section className="panel recent-panel"><header><div><span className="eyebrow">Actividad reciente</span><h2>Negocios incorporados</h2></div><Link to="/negocios">Ver todos <ArrowUpRight /></Link></header>{recent.length ? <div className="business-list">{recent.map((business) => <Link to={`/negocios/${business.id}`} key={business.id}><span className="business-avatar">{business.logo_url ? <img src={business.logo_url} alt="" /> : <Store />}</span><div><strong>{business.name}</strong><small>{business.slug} · Plan {business.plan}</small></div><Status value={business.status} /><ArrowUpRight /></Link>)}</div> : <Empty title="Aún no hay negocios" detail="Crea el primero desde el módulo Negocios." />}</section>
  </div>;
}
